    ...
    <div class="productCenterAll" :class="{'hide':isHide}">
        ...
        <div class="topBanner">
            ...
            <div class="bannerAreaMain">
                ...
                <img src="https://resource.tanwuapp.com/FrSF3siM_uONchU-v6MHRGfssMaa">
            </div>
        </div>
            ...
            <div class="proNameArea">
                <div class="proNameLeft"  v-if="productTypes.length">
                    <span v-for= "(item,index) in productTypes"
                          ...
                          :class="{ active: item.typeId == askData.typeId }">
                        {{item.typeName}}</span>
                </div>
            ...
            <div class="productList">
                <ul>
                    <li v-for="(item,index) in productsList" :key = "(item,index)" >
                        <router-link :to="{path:'/product/productCenterDetail',query:{type_id:item.type_id,product_id:item.product_id}}"  tag="a" target="_blank">
...
.productList ul{width: 1400px;padding: 10px 20px 0 20px;height: auto;overflow: hidden;}
...
.productList ul li .produstOne .proImg{width: 400px;height: 320px;max-height: 320px;background-color: #EDEDED;
    ...
    transition: transform .3s ease;
    transition: transform .3s ease,-webkit-transform .3s ease;}
.productList ul li .produstOne .proImg:hover{
    -webkit-transition: .1s cubic-bezier(.25,.1,.36,1.3);
    transition: .1s cubic-bezier(.25,.1,.36,1.3);
    -webkit-box-shadow: 0 15px 100px -35px #ccc;
    box-shadow: 0 15px 100px -35px #ccc;
    opacity: 1;
    -webkit-transform: scale(1.01);
    transform: scale(1.01);
}
