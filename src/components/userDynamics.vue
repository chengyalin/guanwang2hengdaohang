  ...
  <div class="userDynamics"  @click="open=false" v-title data-title="用户动态" >
      ...
      <div class="topBanner">
          ...
          <div class="bannerAreaMain">
              ...
              <img src="https://resource.tanwuapp.com/lkMxLUen7iEoI2pJG_U4EQLHS4KP">
          </div>
      </div>
          ...
          <div class="userDynamicsList">
              <ul v-if="userDynamicsList.length">
                  <li  v-for="(item,index) in userDynamicsList" :key = "(item,index)">
                      <div class="pullWater">
                          <router-link :to="{path:'/dynamic/userDynamicsDetail',query:{shareId:item.shareId}}"  tag="a" target="_blank">
...
.userDynamicsList{width: 1325px;margin: -50px auto 0;height: auto;overflow: hidden;padding-left: 10px;clear: both;overflow: hidden;}
...
.userDynamicsList ul li:hover .pullWater{box-shadow: 0 0 10px #dedede;}
...
.userDynamicsOne .userInfoTime{font-size: 14px;color:rgba(136,136,136,1);height: 24px;line-height: 24px;border-top: 1px solid rgba(240,240,240,1);padding: 14px 0 20px;margin-bottom: 36px;}
...
.userDynamicsOne .userInfoTime .left .name{display: inline-block;max-width: 60px;overflow: hidden;text-overflow: ellipsis;white-space: nowrap;vertical-align: middle;margin-bottom: 0;float: left;}
